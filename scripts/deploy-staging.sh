...
# 4d. /auth/whoami response (v0.9.10 emergency patch · is_public_path=True · 不再強制 401)
# 改成 warn-only · 因 Edward 5/25 拍板拿掉 Google 登入個人用 (auth_middleware emergency patch)
WHOAMI_CODE=$(curl -s -o /dev/null -w '%{http_code}' "$STAGING_URL/auth/whoami")
if [ "$WHOAMI_CODE" = "401" ]; then
  echo "  ✓ /auth/whoami (no cookie) → 401  (auth 仍 gated)"
elif [ "$WHOAMI_CODE" = "200" ]; then
  echo "  ⚠ /auth/whoami → 200  (v0.9.10 emergency patch · auth 已砍個人用 · 預期)"
else
  echo "  ⚠ /auth/whoami → $WHOAMI_CODE"
fi
