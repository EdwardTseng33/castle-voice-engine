# breeze_poc/run_chatterbox_sophie_mtl.py
# Voice Path v2.0 Phase 1 PoC - Chatterbox-Multilingual driver (CORRECTION RUN)
# 2026-05-22 calcifer · 5/22 上版用 base English-only model · 本次重跑用 MTL + language_id="zh"
# Edward 抓到 bug 後 30-60 min SLA narrow correction
...
OUT_DIR = RESULTS / "chatterbox-turbo"   # OVERWRITE existing (per Edward instruction)
...
# Import deployed Modal class (new MTL app)
import modal
...
meta = {
    ...
    "voice_clone": "zero-shot from prompt mp3 (native Chatterbox MTL API)",
    ...
    "ship_session": "calcifer 5/22 Chatterbox CORRECTION run · MTL model · zh language_id",
    "correction_note": "5/22 上版 t01-t10 用了 base (English-only) ChatterboxTTS · 中文字被英文人硬讀 · Edward 聽出後本次重跑改用 ChatterboxMultilingualTTS + language_id='zh' 修正 · 覆蓋舊版 t01-t10.wav",
    "industry_blind_test_claim": "63.75% prefer over ElevenLabs (per Resemble AI · base model claim, MTL 可能類似但未獨立驗證)",
    "samples": [],
}
...
for i, text in enumerate(sentences, 1):
    ...
    try:
        ...
        print(f"  OK lat={result['latency_ms']}ms inf={result['inference_ms']}ms dur={result['duration_sec']}s sr={result['sample_rate']} lang={result.get('language_id','zh')} clone={result['voice_clone_enabled']} -> {out_wav.name}")
